...
class DatePicker extends Component {
  ...
  constructor(props) {
    ...
    //this.state.dejaDemande:undefined,
    this.state={demandeFaite:false,
		booking:[],
		dejaDemande:[],
		reservations:[],
		dater:new Date().getTime(),}
  }
	...
	traiteLesDates=(arrayDates)=>{
			...
			for(var arraDate of arrayDates){
				if(arraDate.statut==="demande"){
					let froma=new Date(arraDate.range.from); let toa=new Date(arraDate.range.to);
					arrayDemande.push({from:froma,to:toa});
					}
				else{
					let fromu=new Date(arraDate.range.from); let tou=new Date(arraDate.range.to);
				...
				//console.log(" arrayDemand.from = "+arrayDemande[0].from);
				console.log(" arrayBooked.from = "+arrayBooked[0].to);
				//this.setState({booking:arrayBooked,dejaDemande:arrayDemande});
				this.setState({dejaDemande:arrayDemande});
...
/*** Gestion rendering du contenu du component ***/
  ...
  render() {
	...
	const { from, to, dejaDemande, } = this.state;
        const modifiers = {
			   highlighted: new Date(),
			   sundays: { daysOfWeek: [0] },
			   start: from, 
